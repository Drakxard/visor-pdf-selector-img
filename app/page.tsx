  ...
  if (!setupComplete) {
    switch (step) {
      case 0: {
        const handlePick = async () => {
          try {
            ...
            try {
              const system = await dir.getDirectoryHandle('system')
              const notas = await system.getDirectoryHandle('notas')
              const file = await notas.getFileHandle('config.json')
              ...
              try {
                ...
                fetch('/api/config', { method: 'POST', body: JSON.stringify(cfg) })
              }
            } catch {
              setStep(1)
            }
        ...
        return (
          <main className="min-h-screen flex flex-col items-center justify-center gap-4 p-4">
            ...
            <p>Paso 0: Selecciona tu carpeta "gestor"</p>
            <button className="px-4 py-2 border rounded" onClick={handlePick}>
              Elegir carpeta
            </button>
          </main>
        )
      ...
      case 5: {
        ...
        return (
          <main className="min-h-screen flex flex-col items-center justify-center gap-4 p-4">
            <p>Paso 5: Finaliza la configuración</p>
            <p>Se guardará la configuración en la carpeta seleccionada.</p>
