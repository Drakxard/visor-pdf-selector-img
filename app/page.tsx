...
useEffect(() => {
  ...
  const handleDropLink = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    const files = Array.from(e.dataTransfer.files || []).filter((f) =>
      f.name.toLowerCase().endsWith('.lnk') || f.name.toLowerCase().endsWith('.url'),
    )
    if (files.length) {
      const file = files[0]
      Object.defineProperty(file, 'webkitRelativePath', {
        value: `root/Semana${viewWeek}/${viewSubject}/${category}/${file.name}`,
      })
      setDirFiles((prev) => [...prev, file])
      const path = `Semana${viewWeek}/${viewSubject}/${category}/${file.name}`
      const pdf: PdfFile = {
        file,
        path,
        week: viewWeek!,
