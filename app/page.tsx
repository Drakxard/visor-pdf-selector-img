...
export default function Home() {
  ...
  if (!started) {
    ...
    return (
      <main className="min-h-screen flex items-center justify-center text-2xl bg-white dark:bg-gray-900 text-black dark:text-white">
  ...
  if (!setupComplete) {
    switch (step) {
      case 0: {
        return (
          <main className="min-h-screen flex flex-col items-center justify-center gap-4 p-4 bg-white dark:bg-gray-900 text-black dark:text-white">
      ...
      case 4: {
        ...
        return (
          <main className="min-h-screen flex flex-col items-center gap-4 p-4 bg-white dark:bg-gray-900 text-black dark:text-white">
  ...
  const subjects =
    names.length > 0
      ...
      : Array.from(
          new Set(
            Object.values(fileTree).flatMap((s) => Object.keys(s)),
          ),
        )

  const colorMap = subjects.reduce<Record<string, string>>((acc, n, i) => {
    const palette = [
      "bg-red-500",
      "bg-green-500",
      "bg-blue-500",
                ...
                <ul>
                  {pendingFor(selectedDay, filterSubject).map((f) => (
                    <li key={f.path} className="truncate" title={f.file.name}>
                      {f.file.name} - Faltan {daysUntil(f)} días
                    </li>
                  ))}
                </ul>
              ) : null
  ...
  return (
    <>
      ...
      <main className="grid grid-cols-2 min-h-screen bg-white dark:bg-gray-900 text-black dark:text-white">
    ...
    <div className="fixed top-2 right-2">
      ...
      {showSettings && (
        <div className="absolute right-0 mt-2 bg-white dark:bg-gray-800 text-black dark:text-white border p-2 space-y-2">
