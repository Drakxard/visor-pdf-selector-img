    ...
    for await (const [name, handle] of (directory as any).entries()) {
      if (handle.kind === "file") {
        ...
        try {
          ...
          console.warn("Skipping directory", name, err)
        }
      }
    }
  }
...
useEffect(() => {
  ...
  const handleDropLink = async (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (dropped) {
      ...
      const safeName = sanitizeSegment(dropped.name)
      const file = new File([buf], safeName, { type: dropped.type })
      Object.defineProperty(file, 'webkitRelativePath', {
    ...
    const fileName = sanitizeSegment(rawName)
