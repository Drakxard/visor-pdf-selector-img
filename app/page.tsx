...
export default function Home() {
  ...
  const isLinkName = (name: string) =>
    name.toLowerCase().endsWith('.lnk') || name.toLowerCase().endsWith('.url')
  ...
  const selectDirectory = async () => {
    try {
      ...
      setNames([])
      setDirFiles(files)
      if (skipped.some((n) => isLinkName(n))) {
        setToast({ type: 'error', text: 'No se pudieron leer archivos de enlace' })
        if (toastTimerRef.current) window.clearTimeout(toastTimerRef.current)
        toastTimerRef.current = window.setTimeout(() => setToast(null), 3000)
      }
...
useEffect(() => {
  ...
  const handleDropLink = async (e: React.DragEvent<HTMLDivElement>) => {
    ...
    const fileName =
      name.endsWith('.lnk') || name.endsWith('.url') ? name : `${name}.url`
  ...
  const handleAddLink = async () => {
    if (!newLink.trim() || !viewWeek || !viewSubject) return
    const url = newLink.trim()
    const base = url.includes('youtube') ? 'video' : 'enlace'
    const fileName = `${base}-${Date.now()}.url`
    const content = `[InternetShortcut]\nURL=${url}\n`
    const file = new File([content], fileName, { type: 'text/plain' })
    Object.defineProperty(file, 'webkitRelativePath', {
        ...
        {viewWeek && viewSubject && (
          <>
            ...
            <div className="flex flex-wrap gap-2 my-2">
              ...
              <select
                value={linkCategory}
                onChange={(e) => setLinkCategory(e.target.value as 'theory' | 'practice')}
