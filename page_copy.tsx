...
const matchesCategorySegment = (segment: string, category: 'theory' | 'practice') => {
  const key = normalizeKey(segment)
  if (category === 'theory') {
    return key.includes('teo') || key.includes('theor')
  }
  return key.includes('prac')
}

const extractSubjectContext = (path: string) => {
  const segments = path.split('/').filter(Boolean)
  if (!segments.length) {
    return { basePath: '', subjectName: '', subjectKey: '' }
  }
  let baseSegments = [...segments]
  let subjectName = baseSegments[baseSegments.length - 1] || ''
  const lastKey = normalizeKey(subjectName)
  if (matchesCategorySegment(subjectName, 'theory') || matchesCategorySegment(subjectName, 'practice')) {
    baseSegments = baseSegments.slice(0, -1)
    subjectName = baseSegments[baseSegments.length - 1] || subjectName
  }
  if (baseSegments.length >= 2 && normalizeKey(baseSegments[0]).startsWith('semana')) {
    subjectName = baseSegments[1] || subjectName
    baseSegments = baseSegments.slice(0, 2)
  }
...
const upgradeLegacyConfig = (item: any): MoodleFolderConfig | null => {
  if (!item || typeof item !== 'object') return null
  if (typeof item.basePath === 'string' && typeof item.category === 'string') {
    const { basePath } = item
    const context = extractSubjectContext(basePath)
    return {
      id:
      ...
      basePath,
      category: item.category === 'practice' ? 'practice' : 'theory',
      subjectKey: typeof item.subjectKey === 'string' && item.subjectKey
        ? item.subjectKey
        : context.subjectKey,
      subjectName: typeof item.subjectName === 'string' && item.subjectName
        ? item.subjectName
        : context.subjectName || 'Materia',
      courseId: Number(item.courseId) || 0,
      folderId: Number(item.folderId) || 0,
...
export default function Home() {
  ...
  const isSubjectLevel =
    !!currentSubjectContext.basePath && currentSubjectContext.basePath === currentDirEntry.path
